...
class TestStartPublishing:
    ...
    def test_start_publishing_success(self):
        ...
        with patch(
            "rpi_usb_cloner.services.discovery.Zeroconf",
            return_value=mock_zeroconf,
        ), patch.object(service, "_get_local_ip", return_value="192.168.1.50"), patch(
            "socket.gethostname", return_value="testpi"
        ):
    ...
    def test_start_publishing_no_ip(self):
        ...
        with patch.object(service, "_get_local_ip", return_value=None), pytest.raises(
            RuntimeError, match="Could not determine local IP"
        ):
            service.start_publishing(mock_pin_callback)
...
class TestParseServiceInfo:
    ...
    def test_parse_error_handling(self):
        ...
        type(mock_info).addresses = property(
            lambda _: (_ for _ in ()).throw(Exception("test"))
        )
...
class TestGetLocalIP:
    ...
    def test_get_local_ip_fallback_to_hostname(self):
        ...
        with patch("socket.socket", side_effect=Exception("Socket failed")), patch(
            "socket.gethostname", return_value="testhost"
        ), patch("socket.gethostbyname", return_value="192.168.1.60"):
            ip = service._get_local_ip()
    ...
    def test_get_local_ip_fallback_loopback_filtered(self):
        ...
        ), patch("socket.gethostbyname", return_value="127.0.0.1"), patch(
            "rpi_usb_cloner.services.discovery.log"
        ):
            ip = service._get_local_ip()
    ...
    def test_get_local_ip_all_methods_fail(self):
        ...
        with patch("socket.socket", side_effect=Exception("Socket failed")), patch(
            "socket.gethostname", side_effect=Exception("Hostname failed")
        ), patch("rpi_usb_cloner.services.discovery.log"):
            ip = service._get_local_ip()
