...
def prompt_text(
    ...
    *,
    ...
    while True:
        _render_keyboard(title, value, masked, layout, selected_row, selected_col)
        now = time.monotonic()
        current_u = read_button(PIN_U)
        if prev_states["U"] and not current_u:
            selected_row = max(0, selected_row - 1)
            selected_col = min(selected_col, len(layout.rows[selected_row]) - 1)
        ...
        if prev_states["D"] and not current_d:
            selected_row = min(len(layout.rows) - 1, selected_row + 1)
            selected_col = min(selected_col, len(layout.rows[selected_row]) - 1)
        ...
        if prev_states["R"] and not current_r:
            selected_col = min(len(layout.rows[selected_row]) - 1, selected_col + 1)
        ...
        if prev_states["B"] and not current_b:
            key = layout.rows[selected_row][selected_col]
