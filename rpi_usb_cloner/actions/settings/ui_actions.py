...
def toggle_menu_view_mode() -> None:
    """Toggle menu view mode between list and icon."""
    current_mode = settings.get_setting("menu_view_mode", default="list")
    # Toggle between list and icon
    new_mode = "icon" if current_mode == "list" else "list"
    settings.set_setting("menu_view_mode", new_mode)

    mode_label = "ICON VIEW" if new_mode == "icon" else "LIST VIEW"
    screens.render_status_template("VIEW MODE", f"Set to {mode_label}")
    time.sleep(1.5)
