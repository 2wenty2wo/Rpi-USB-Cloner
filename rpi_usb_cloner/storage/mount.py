...
def get_partition(device):
    ...
    try:
        ...
        result = subprocess.run(["fdisk", "-l", device], check=True, capture_output=True, text=True)

        # Parse output to find last partition
        lines = result.stdout.splitlines()
        partition_lines = [line.strip() for line in lines if line.strip().startswith("/dev/")]
        if partition_lines:
...
def mount_partition(partition, name="usb"):
    ...
    if not is_mounted(path):
        try:
            ...
            subprocess.run(["mkdir", "-p", path], check=True, capture_output=True, text=True)

            # Mount the partition using subprocess
            subprocess.run(["mount", partition, path], check=True, capture_output=True, text=True)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Failed to mount {partition} to {path}: {e.stderr.strip()}") from e
