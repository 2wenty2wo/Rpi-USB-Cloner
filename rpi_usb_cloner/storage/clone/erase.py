...
def erase_device(target, mode, progress_callback=None):
    ...
    try:
        ...
        logger.error(f"Erase aborted: {error}")
        return False
        ...
        logger.error(f"Erase aborted: validation failed: {error}")
        return False
    ...
    with device_operation(target_name):
        if not unmount_device(target):
            ...
            logger.error("Erase aborted: target unmount failed")
            return False
        try:
        ...
        def run_erase_command(command, total_bytes=None):
            try:
                ...
                logger.error(f"Erase command failed: {e}")
                return False
        ...
        if mode == "secure":
            ...
            if not shred_path:
                ...
                logger.error("Erase failed: shred not available")
                return False
        ...
        if mode == "discard":
            ...
            if not discard_path:
                ...
                logger.error("Erase failed: blkdiscard not available")
                return False
        ...
        if mode == "zero":
            ...
            if not dd_path:
                ...
                logger.error("Erase failed: dd not available")
                return False
            return run_erase_command(
                [
                    dd_path,
        ...
        if mode != "quick":
            ...
            logger.error(f"Erase failed: unknown mode {mode}")
            return False
        ...
        if not wipefs_path:
            ...
            logger.error("Erase failed: wipefs not available")
            return False
        dd_path = shutil.which("dd")
        if not dd_path:
            emit_error("no dd tool")
        ...
        wipe_mib = coerce_int(
            min(quick_wipe_mib, size_mib) if size_mib else quick_wipe_mib
        )
